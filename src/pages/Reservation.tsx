...
const DEFAULT_AIRLINES = [
  { id: "korean-air", name: "대한항공", group: "domestic" },
  { id: "asiana", name: "아시아나항공", group: "domestic" },
  { id: "jin-air", name: "진에어", group: "domestic" },
  { id: "jeju-air", name: "제주항공", group: "domestic" },
  { id: "tway", name: "티웨이항공", group: "domestic" },
  { id: "air-busan", name: "에어부산", group: "domestic" },
  { id: "air-seoul", name: "에어서울", group: "domestic" },
  { id: "eastar", name: "이스타항공", group: "domestic" },
  { id: "air-premia", name: "에어프레미아", group: "domestic" },
  { id: "vietnam-airlines", name: "베트남항공", group: "international" },
  { id: "other", name: "기타 (직접 입력)", group: "other", allowCustom: true }
];
...
export const Reservation = () => {
  ...
  // Shared resources from the dynamic import
  const [sharedModule, setSharedModule] = useState<any>(null);
  const [airlinesData, setAirlinesData] = useState<any[]>(DEFAULT_AIRLINES);
  ...
  // Load shared module on mount
  useEffect(() => {
    const loadSharedResources = async () => {
      try {
        const moduleUrl = 'https://airpick-b2c.web.app/shared/submit-homepage-reservation.mjs';
        const mod = await import(/* @vite-ignore */ moduleUrl);
        setSharedModule(mod);
        if (mod.fetchAirlines) {
          const data = await mod.fetchAirlines();
          if (data && data.airlines) {
            setAirlinesData(data.airlines);
          }
        }
      } catch (err) {
        console.error('Failed to load shared reservation module, trying direct fetch:', err);
        try {
          const res = await fetch('https://airpick-b2c.web.app/shared/airlines.json');
          if (res.ok) {
          ...
          console.error('Airlines JSON direct fetch also failed:', innerErr);
        }
      }
    };
    loadSharedResources();
  ...
  const handlePhoneInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (sharedModule?.formatPhoneInput) {
      setFormData({ ...formData, phone: sharedModule.formatPhoneInput(e.target.value) });
    } else {
      // Fallback format matching B2C spec: 010-0000-0000
  ...
  const handlePasswordInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData({ ...formData, reservationPassword: numeric });
  };
  ...
  const isFormComplete = 
    ...
    formData.reservationPassword.length === 4 &&
    agreeTerms &&
    agreePrivacy &&
    totalPrice !== null;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Prepare standard B2C form & search objects
    const formPayload = {
      userName: formData.name,
      phone: formData.phone,
      carModel: formData.carModel,
      carNumber: formData.carNumber,
      departureAirlineId: formData.entryAirlineId,
      departureAirline: formData.entryAirline,
      departureAirlineCustom: formData.entryAirlineCustom,
      departureFlight: formData.entryFlight,
      arrivalAirlineId: formData.exitAirlineId,
      arrivalAirline: formData.exitAirline,
      arrivalAirlineCustom: formData.exitAirlineCustom,
      arrivalFlight: formData.exitFlight,
      reservationPassword: formData.reservationPassword,
    ...
    const searchPayload = {
      ...
      terminal: formData.entryTerminal,
      arrivalDate: ensureYYYYMMDD(formData.exitDate),
      arrivalTime: finalExitTimeStr,
      arrivalTerminal: formData.exitTerminal || formData.entryTerminal,
      isIndoor: formData.parkingType === 'indoor',
    };

    // 1. Validation with official mjs module or exact fallback
    let errorMsg: string | null = null;
    if (sharedModule?.validateHomepageReservation) {
      errorMsg = sharedModule.validateHomepageReservation({ form: formPayload, search: searchPayload });
    } else {
      if (!formPayload.userName.trim()) errorMsg = '이름을 입력해 주세요.';
      else if (!formPayload.phone.trim()) errorMsg = '연락처를 입력해 주세요.';
      else if (!formPayload.carModel.trim()) errorMsg = '차량 모델을 입력해 주세요.';
      else if (!formPayload.carNumber.trim()) errorMsg = '차량번호를 입력해 주세요.';
      else if (!/^\d{4}$/.test(formPayload.reservationPassword.trim())) {
        errorMsg = '예약 비밀번호는 숫자 4자리입니다.';
      }
      else if (!searchPayload.departureDate || !searchPayload.arrivalDate) {
        errorMsg = '입고·출고 일정을 선택해 주세요.';
      }
      else if (!searchPayload.departureTime || !searchPayload.arrivalTime) {
        errorMsg = '입고·출고 시간을 선택해 주세요.';
      }
      else if (!searchPayload.terminal) errorMsg = '출국 터미널을 선택해 주세요.';
      else {
        const resolveAirlineNameLocal = (id: string, name: string, custom: string) => {
          if (id === 'other') return custom.trim();
          return name.trim() || custom.trim();
        };
        const depAirline = resolveAirlineNameLocal(formPayload.departureAirlineId, formPayload.departureAirline, formPayload.departureAirlineCustom);
        const arrAirline = resolveAirlineNameLocal(formPayload.arrivalAirlineId, formPayload.arrivalAirline, formPayload.arrivalAirlineCustom);
        
        if (!depAirline) errorMsg = '출국 항공사를 선택해 주세요.';
        else if (!formPayload.departureFlight.trim()) errorMsg = '출국 편명을 입력해 주세요.';
        else if (!arrAirline) errorMsg = '입국 항공사를 선택해 주세요.';
        else if (!formPayload.arrivalFlight.trim()) errorMsg = '입국 편명을 입력해 주세요.';
      }
    }
    ...
    // 2. ID Creation
    let id = `res_${Date.now()}`;
    if (sharedModule?.createReservationId) {
      id = sharedModule.createReservationId();
    }

    // 3. Payload Building
    let finalPayload: any;
    if (sharedModule?.buildHomepageReservationPayload) {
      finalPayload = sharedModule.buildHomepageReservationPayload({
        companyId: 'wawa',
        companyName: '와와발렛',
        form: formPayload,
      ...
      const departureAirline = resolveAirlineNameLocal(formPayload.departureAirlineId, formPayload.departureAirline, formPayload.departureAirlineCustom);
      const arrivalAirline = resolveAirlineNameLocal(formPayload.arrivalAirlineId, formPayload.arrivalAirline, formPayload.arrivalAirlineCustom);
      
      const createReceiptTokenLocal = () => {
      ...
      finalPayload = {
        ...
        phone: formPayload.phone.trim(),
        carModel: formPayload.carModel.trim(),
        carNumber: formPayload.carNumber.trim(),
        departureDate: searchPayload.departureDate,
        departureTime: searchPayload.departureTime,
        departureTerminal: searchPayload.terminal,
        arrivalDate: searchPayload.arrivalDate,
        arrivalTime: searchPayload.arrivalTime,
        arrivalTerminal: searchPayload.arrivalTerminal,
        departureAirline,
        departureFlight: formPayload.departureFlight.trim().toUpperCase(),
        arrivalAirline,
        arrivalFlight: formPayload.arrivalFlight.trim().toUpperCase(),
        reservationPassword: formPayload.reservationPassword.trim(),
        totalPrice: Math.round(Number(totalPrice) || 0),
        status: 'pending',
        createdAt: new Date().toISOString(),
        createdBy: 'homepage',
        notifyOnCreate: true,
        receiptToken: createReceiptTokenLocal(),
        paymentMethod: 'unpaid',
        isIndoor: searchPayload.isIndoor !== false,
        scratchPhotos: { synced: false },
      };
      if (formPayload.destination.trim()) finalPayload.destination = formPayload.destination.trim();
      if (formPayload.customerNotes.trim()) finalPayload.customerNotes = formPayload.customerNotes.trim();
    }
    ...
    try {
      ...
      await setDoc(doc(collection(db, reservationsCol), id), finalPayload);
      
      // Google Ads Conversion tracking upon successful submission
      try {
                ...
                <div className="grid sm:grid-cols-3 gap-4 border-t border-white/5 pt-5">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <select
                      ...
                      value={formData.entryAirlineId}
                      onChange={(e) => {
                        const val = e.target.value;
                        const matched = airlinesData.find(a => (a.allowCustom ? 'other' : a.name) === val);
                        setFormData({
                          ...formData,
                          entryAirlineId: val,
                          entryAirline: val === 'other' ? '' : (matched ? matched.name : val)
                        });
                      }}
                    >
                      {[
                        { id: "domestic", label: "국내 항공사" },
                        { id: "international", label: "국제선 (인천 자주 이용)" },
                        { id: "other", label: "기타" }
                      ].map(group => {
                        const groupAirlines = airlinesData.filter(a => a.group === group.id);
                        if (groupAirlines.length === 0) return null;
                        return (
                          <optgroup key={group.id} label={group.label} className="bg-slate-950 text-slate-400">
                            {groupAirlines.map(airline => {
                              const val = airline.allowCustom ? 'other' : airline.name;
                              return (
                                <option key={airline.id} value={val} className="text-white bg-slate-950">
                                  {airline.name}
                                </option>
                              );
                            })}
                    ...
                    {formData.entryAirlineId === 'other' && (
                      <input 
                        type="text"
                        required
                        placeholder="항공사명 직접 입력"
                        className="w-full mt-2 bg-slate-950 border border-white/10 rounded-xl px-4 py-3 placeholder:text-slate-500 focus:ring-1 focus:ring-[#FFD500] focus:border-[#FFD500] outline-none font-bold text-white text-sm transition-all"
                        value={formData.entryAirlineCustom}
                        onChange={(e) => setFormData({...formData, entryAirlineCustom: e.target.value, entryAirline: e.target.value})}
                      />
                    )}
                  </div>
              ...
              <div className="space-y-6">
                ...
                <div className="grid sm:grid-cols-3 gap-4 border-t border-white/5 pt-5">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        setFormData({
                          ...formData,
                          exitAirlineId: val,
                          exitAirline: val === 'other' ? '' : (matched ? matched.name : val)
                        });
                      }}
                    >
                      {[
                    ...
                    {formData.exitAirlineId === 'other' && (
                      <input 
                        type="text"
                        required
                        placeholder="항공사명 직접 입력"
                        className="w-full mt-2 bg-slate-950 border border-white/10 rounded-xl px-4 py-3 placeholder:text-slate-500 focus:ring-1 focus:ring-[#FFD500] focus:border-[#FFD500] outline-none font-bold text-white text-sm transition-all"
                        value={formData.exitAirlineCustom}
                        onChange={(e) => setFormData({...formData, exitAirlineCustom: e.target.value, exitAirline: e.target.value})}
                      />
                    )}
                  </div>
              ...
              <div className="grid sm:grid-cols-2 gap-6">
                ...
                <div className="space-y-1.5 sm:col-span-1">
                  ...
                  <input 
                    ...
                    maxLength={4}
                    className="w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-3 placeholder:text-slate-500 focus:ring-1 focus:ring-[#FFD500] focus:border-[#FFD500] outline-none font-bold text-white text-sm transition-all"
                    value={formData.reservationPassword}
              ...
              <button 
                ...
                onClick={() => {
                  try {
                    ...
                    console.error('gtag button click conversion error:', gtagErr);
                  }
                }}
